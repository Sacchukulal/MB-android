...
/**
 * The floor, from the phone: the counter's catalogue (cached), the orders this phone has
 * touched, and the ONE way anything reaches the counter — an intent, written to the database
 * before its first send, sent, and answered with an outcome that is final.
 */
@Singleton
class Floor @Inject constructor(
    ...
    /**
     * The one door. The row is durable before the first send; a lost reply is retried by
     * [flush] with the SAME id; the outcome, whatever it is, is final.
     */
     ...
     * durable intents and sent as ONE batch; the counter carries the new order's id through
     * the batch. One round trip, and no 0.00 ghost order if the waiter walks away half-way.
     */
    suspend fun stageOrder(orderId: String?, place: Place, lines: List<StagedLine>, note: String?): Answer<BatchResult?> {
        ...
        val rows = buildList {
            if (orderId == null) {
                add(IntentRow(newId(), null, now, Ops.openOrder(place.orderType, place.tableId, null).toString(), "Open $whereLabel", place.tableLabel, "queued", null, now, null, 0))
            }
            lines.forEach { l ->
        ...
        rows.forEach { db.intents().put(it) }
        return when (val a = flush()) {
            is Answer.Ok -> a
            is Answer.Unreachable -> Answer.Refused("Could not reach the counter — the order is queued and goes the moment it is back.")
            is Answer.Refused -> a
    ...
    private suspend fun record(row: IntentRow, outcome: Outcome, place: Place?) {
        ...
        if (outcome is Outcome.Ok) {
            ...
            if (finished.any { outcome.message.contains(it, ignoreCase = true) }) {
                db.floor().order(row.orderId)?.let { db.floor().putOrder(it.copy(closedSays = outcome.message, updatedMs = clock.now())) }
            }
        }
    }
    ...
    /**
     * The floor as the counter describes it: every table's state, every open order. The orders
     * this phone holds are brought up to date; one that is no longer open is finished with.
     */
    suspend fun takeFloor(body: JsonObject) {
        ...
        val mine = db.floor().openOrders().first()
        for (row in mine) {
            val o = open[row.orderId]
            if (o == null) {
                db.floor().putOrder(row.copy(closedSays = "The counter has finished with this order. Start a new one for anything else.", updatedMs = now))
            } else {
                db.floor().putOrder(
                    row.copy(
                        ...
                        total = o.strOrNull("total") ?: row.total,
    ...
    /** On opening the floor: the snapshot, so the tiles are right before the first push. */
